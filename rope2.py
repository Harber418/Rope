...
class Rope:
    ...
    def __init__(self, N, m, g, k, rest_L, M, M_pos, anchor, dt, time,
        ...
        self.year = 2
        self.puv = 0 + self.year*0.15
        self.creep = 0 +self.year*0.125
        self.k = k*(1-min(self.puv,0.65)*(1-min(self.creep,0.65)))
        ...
        self.run(0.5) # allow the rope to equilibriate before the fall
    ...
    def save_history(self, filename, fall_factor):
        ...
        np.savez(filename, p_hist=np.array(self.p_hist), f_hist=np.array(self.f_hist), v_hist=np.array(self.v_hist), fall_factor=fall_factor, masses=self.masses,time = self.timesteps * self.dt)
...
if __name__ == "__main__":
    main(100, 3, 70000, 5, 75, np.array([0,5]), 5, 0.001, 10, 0, 1.08)     
